...
def get_provider_details(owner_id):
    ...
    try:
        ...
        provider_verified = owner.get("verified", owner.get("is_verified", True))
...
def get_provider_pickup_location(owner_id):
    ...
    try:
        ...
        area = str(provider.get("area", "")).strip()
...
def create_listing():
    ...
    if not session_user_id:
        ...
        return (
            jsonify({"success": False, "message": "Authentication is required."}),
            401,
        )
    ...
    listing_type = str(payload.get("listing_type", "")).strip().lower()

    if listing_type not in {"sell", "donate"}:

        return (
            jsonify(
                {
                    "success": False,
                    "message": ("Listing type must be either " "'sell' or 'donate'."),
                }
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "message": ("Quantity must be a whole number.")}
            ),
            400,
        )
            ...
            jsonify(
                {
                    "success": False,
                    "message": (
                        "MongoDB is currently unavailable. " "Please try again later."
                    ),
                }
            ),
            500,
    ...
    if donations_collection is None:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": ("MongoDB donations collection is unavailable."),
                }
            ),
            500,
        )
    ...
    listing_document = {field: payload[field] for field in REQUIRED_FIELDS}
    ...
    listing_document.update(
        {
            ...
            "pickup_instructions": (
                ...
                if isinstance(payload.get("pickup_instructions", ""), str)
                else payload.get("pickup_instructions", "")
            ),
            "preparation_time": (
                payload.get("preparation_time", "").strip()
                if isinstance(payload.get("preparation_time", ""), str)
                else payload.get("preparation_time", "")
            ),
            "freshness_score": payload.get("freshness_score", 0),
            "recovery_probability": payload.get("recovery_probability", 0),
            "carbon_saved": payload.get("carbon_saved", 0),
            "ai_recommendation": payload.get(
                "ai_recommendation", "No additional recommendation."
            )
            ...
            jsonify(
                {
                    "success": False,
                    "message": (
                        "The listing could not be saved. " "Please try again later."
                    ),
                }
            ),
            500,
    ...
    if listing_type == "donate":
        ...
        try:
            ...
            donation_document = {
                ...
                # Food information
                "food_title": payload.get("food_title", ""),
                ...
                "donation_pickup_start": normalize_datetime(payload["pickup_start"]),
                "donation_pickup_end": normalize_datetime(payload["pickup_end"]),
                # Donation status
                "status": "available",
            ...
            donation_result = donations_collection.insert_one(donation_document)
            ...
            print("MongoDB donation creation error:", error)
            ...
            try:
                ...
                print("MongoDB donation cleanup error:", cleanup_error)
    ...
    if listing_type == "donate":
        ...
        return (
            jsonify(
                {
                    "success": True,
                    "message": ("Food donation published " "successfully for NGOs."),
...
def get_public_listings():
    ...
    if listings_collection is None:
        ...
        return (
            jsonify(
                {"success": False, "message": ("MongoDB is currently unavailable.")}
            ),
            500,
        )
    ...
    query = {"status": "available", "listing_type": "sell", "quantity": {"$gt": 0}}
    ...
    try:
        ...
        food_listings = list(listings_collection.find(query).sort("created_at", -1))

        listings_data = []

        for listing in food_listings:

            provider = get_provider_details(listing.get("owner_id"))
            ...
            listings_data.append(
                {
                    ...
                    "preparation_time": listing.get("preparation_time", ""),
                    ...
                    # AI
                    "freshness_score": listing.get("freshness_score", 0),
                    "recovery_probability": listing.get("recovery_probability", 0),
                    "carbon_saved": listing.get("carbon_saved", 0),
                    "ai_recommendation": listing.get("ai_recommendation", ""),
                    # Status
                    "status": listing.get("status", "available"),
                    # Statistics
                    "views": listing.get("views", 0),
                    "reservations": listing.get("reservations", 0),
                    # Date
                    "created_at": (
        ...
        print("MongoDB public listings error:", error)

        return (
            jsonify({"success": False, "message": ("Unable to load food listings.")}),
            500,
        )
...
def get_my_listings():
    ...
    try:
        ...
        user_listings = list(
            listings_collection.find({"owner_id": owner_id}).sort("created_at", -1)
        )
        ...
        listings_data.append(
            {
                ...
                "donation_id": (
                    str(listing["donation_id"]) if listing.get("donation_id") else None
                ),
            }
        )
    ...
    return (
        jsonify(
            {"success": True, "count": len(listings_data), "listings": listings_data}
        ),
        200,
    )
...
@listings.route("/api/listings/<listing_id>/view", methods=["POST"])
def record_listing_view(listing_id):
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Invalid user or listing ID."}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": "Unable to record listing view."}),
            500,
        )
    ...
    return jsonify({"success": True, "views": listing.get("views", 0)}), 200
...
def get_listing(listing_id):
    ...
    try:
        ...
        print("MongoDB public single listing error:", error)

        return jsonify({"success": False, "message": ("Unable to load listing.")}), 500
...
def update_listing(listing_id):
    ...
    try:
        ...
        existing_listing = listings_collection.find_one(
            {"_id": listing_object_id, "owner_id": owner_id}
        )
        ...
        return (
            jsonify(
                {"success": False, "message": ("Unable to load listing for update.")}
            ),
            500,
        )
    ...
    current_status = str(existing_listing.get("status", "available")).lower()
    ...
    existing_type = str(existing_listing.get("listing_type", "")).strip().lower()

    submitted_type = str(payload.get("listing_type", existing_type)).strip().lower()
    ...
    try:
        ...
        update_quantity = int(update_data.get("quantity", 0))
    ...
    if "preparation_time" in payload:
        ...
        if payload.get("preparation_time"):
            ...
            update_data["preparation_time"] = str(payload["preparation_time"]).strip()
    ...
    update_data["area"] = pickup_location["area"]
    ...
    try:
        ...
        result = listings_collection.update_one(
            {"_id": listing_object_id, "owner_id": owner_id}, {"$set": update_data}
        )
            ...
            jsonify(
                {
                    "success": False,
                    "message": (
                        "The listing could not be updated. " "Please try again later."
                    ),
                }
            ),
            500,
    ...
    if result.matched_count == 0:
        ...
        return (
            jsonify({"success": False, "message": ("Listing could not be updated.")}),
            404,
        )
    ...
    if existing_type == "donate":
        ...
        if donations_collection is not None:
            ...
            try:
                    ...
                    {"listing_id": listing_object_id},
                    {
                        "$set": {
                            ...
                            "unit": update_data.get(
                                "unit", existing_listing.get("unit", "")
                            ),
                            "food_title": update_data.get(
                                "food_title", existing_listing.get("food_title", "")
                            ),
                            "category": update_data.get(
                                "category", existing_listing.get("category", "")
                            ),
                            "food_type": update_data.get(
                                "food_type", existing_listing.get("food_type", "")
                            ),
                            "description": update_data.get(
                                "description", existing_listing.get("description", "")
                            ),
                            "preparation_time": update_data.get(
                                "preparation_time",
                                existing_listing.get("preparation_time", ""),
                            ),
                            "image": update_data.get(
                                "image", existing_listing.get("image", "")
                            ),
                            "pickup_address": (pickup_location["address"]),
                            "city": (pickup_location["city"]),
                            "pickup_instructions": str(
                                update_data.get("pickup_instructions", "")
                            ).strip()[:250],
                            "donation_pickup_start": (update_data["pickup_start"]),
                            "donation_pickup_end": (update_data["pickup_end"]),
                            "updated_at": datetime.now(timezone.utc),
                        }
                    },
                )
                ...
                print("MongoDB donation sync error:", error)
...
def toggle_pause_listing(listing_id):
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": ("Unable to update listing.")}),
            500,
        )
    ...
    current_status = str(listing.get("status", "available")).strip().lower()
    ...
    return jsonify({"success": True, "message": message, "status": new_status}), 200
...
@listings.route("/api/listings/<listing_id>/duplicate", methods=["POST"])
def duplicate_listing(listing_id):
    ...
    original_title = str(original_listing.get("food_title", "Food Listing")).strip()
    ...
    try:
        ...
        result = listings_collection.insert_one(duplicate)
                ...
                jsonify(
                    {
                        "success": False,
                        "message": (
                            "Listings with donation history " "cannot be deleted."
                        ),
                    }
                ),
                409,
            ...
            jsonify(
                {
                    "success": False,
                    "message": (
                        "The listing could not be deleted. " "Please try again later."
                    ),
                }
            ),
            500,
